...
//only for general
if (!empty($_POST['location'])) $location = filter_var($_POST['location'],FILTER_SANITIZE_STRING);   else $location ="";
...
if (!empty($_POST['title']))        $title        = htmLawed($_POST['title']);
...
$insert_specs    = serialize(array('phone'=>$phone,'location'=>$location,'address'=>$address,'city'=>$city,'county'=>$county,'state'=>$state,'price'=>$price,'email'=>$email,'website1'=>$website1,'website2'=>$website2,'website3'=>$website3));
...
if (!empty($id)) {
$query = 'UPDATE general SET `location`=\''.$insert_specs.'\', `description`=\''.$insert_descr.'\', `image`=\''.$insert_images.'\', `u_date`=\''.$insert_dates.'\', `template`=\''.$insert_template.'\' WHERE `id`="'.$id.'"';
$db_message = "<h1>The Database Has Been Updated</h1>";
$result = mysql_query($query) or die("<b>A fatal MySQL error occured</b>.\n<br />Query: " . $query . "<br />\nError: (" . mysql_errno() . ") " . mysql_error());
}//end if post db_id isset
else {
$query = "INSERT INTO general (id,location,description,image,u_date,template)
...
$template = "general".$_POST['template'].".php";
...
if (isset($location)) { echo'<input type="hidden" id="location"     name="location"     value="'.$location.'">'."\n"; }
...
<?php
$next_increment    = 0;
$qShowStatus       = "SHOW TABLE STATUS LIKE 'general'";
...
?>
<form action="../general.php" method="post" target="general" name="reload">
